...
`engine="polars"`. The repaired source patch is installed only in candidate workers.

Each backend is exercised in two dependency lanes:
...
the checked-in `.in` files are the human-reviewed direct constraints.
...
Both workers now point at clean checkouts of the same `v2.5.1` commit. The local version patch
reports only the repaired candidate as `pytimetk==2.5.1+parity.1`, so report provenance cannot
confuse it with stock 2.5.1. The semantic patch remains separately upstreamable.

Regenerate the synthetic fixtures and verify that the configs load:
...
Run the complete, unfiltered ledger in each lane:
