...
def _campaign(
    *,
    ...
    for generated in deterministic:
        ...
        reference, candidate, mismatches, status = _observe_pair(
            generated.table,
            reference_runner,
            candidate_runner,
            comparison,
    ...
    if not exact_only and not failures:
        ...
        def differs(table: pa.Table) -> bool:
            ...
            generated_count += 1
            latest_table = table
            latest = _observe_pair(
                table, reference_runner, candidate_runner, comparison, pair_runner
            )
...
def _configured_case(
    ...
    *,
    ...
    with (
        IsolatedExecutionSession(
            case.reference, timeout_seconds=case.timeout_seconds
        ) as reference_session,
        IsolatedExecutionSession(
            case.candidate, timeout_seconds=case.timeout_seconds
    ...
    *,
    ...
    for case in config.cases:
        if selected_cases is not None and case.name not in selected_cases:
            continue
        try:
            result = _configured_case(case, store)
...
def _importable_spec(function: Callable[..., Any], *, adapter: AdapterName) -> CallableSpec | None:
    ...
    try:
        return CallableSpec(target=f"{module}:{qualified}", adapter=adapter, workdir=workdir)
    except ValueError:
        return None
...
def run_live(
    ...
    *,
    ...
    reference_spec = _importable_spec(reference, adapter=reference_adapter)
    candidate_spec = _importable_spec(candidate, adapter=candidate_adapter)
    ...
    def reference_runner(value: pa.Table) -> Observation:
        return execute_callable_current(reference, value, adapter=reference_adapter)

    def candidate_runner(value: pa.Table) -> Observation:
        return execute_callable_current(candidate, value, adapter=candidate_adapter)
...
def replay_artifact(path: str | Path) -> SuiteResult:
    ...
    if replay.get("version") != 1 or not isinstance(replay.get("case"), dict):
